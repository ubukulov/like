        ...
        <div class="col-md-12">
            <div class="box box-primary">
                ...
                <div class="box-body">
                    ...
                    <br>
                    <form action="#" method="post" class="ui large form" enctype="multipart/form-data">
                        {{ csrf_field() }}
                        <input type="hidden" id="money" value="1" name="money"/>
                        <input type="hidden" id="gift" value="1" name="gift"/>
                        <div class="field">
                            <label>Названия</label>
                            <input type="text" name="title" placeholder="Названия" required="required" value="{{ $task->title }}">
                        </div>
                        <div class="field">
                            <label>Описание задании</label>
                            <textarea id="conditions" name="text" rows="4">{{ $task->text }}</textarea>
                        </div>
                        <div class="field">
                            <label>Укажите тип задании</label>
                            <select id="id_type" name="id_type" class="ui search dropdown">
                                @foreach($task_types as $type)
                                    @if($type->id == $task->id_type)
                                    <option selected="selected" value="{{ $type->id }}">{{ $type->name_ru }}</option>
                                    @else
                        ...
                        <div class="two fields">
                            <div class="field">
                                ...
                                <div class="ui small labeled input">
                                    ...
                                    <input type="text" placeholder="mysite.com" name="related_works" value="{{ $task->related_works }}">
                                </div>
                            </div>
                            ...
                            <div class="field">
                                <div class="ui small image">
                                    <div id="image1">
                                        @if(!empty($task->image))
                                        <img src="{{ asset('uploads/tasks/small/'.$task->image) }}">
                                        @else
                        ...
                        <div class="field">
                            <button class="ui large green button" type="button"><i class="save icon"></i>&nbsp;&nbsp;Сохранить задания</button>
                        </div>
