    ...
    b.DEFAULTS = {
        ...
        function h(d, e) {
            ...
            if (e && (f = 'div class="fr-temp-div"'), f)
                for (var g = b.node.contents(d.get(0)), h = null, i = 0; i < g.length; i++) {
                    ...
                    else j.nodeType == Node.TEXT_NODE && 0 == a(j).text().length ? a(j).remove() : (null == h && (h = a("<" + f + ">"), a(j).before(h)), j.nodeType == Node.TEXT_NODE && a(j).text().trim().length > 0 ? (h.append(a(j).clone()), a(j).remove()) : h.append(a(j)))
                }
        }
        ...
        function k() {
            for (var c = d(), e = 0; e < c.length; e++) {
                var f = c[e];
                "false" == f.getAttribute("contenteditable") || 0 != f.querySelectorAll(b.opts.htmlAllowedEmptyTags.join(":not(.fr-marker),") + ":not(.fr-marker)").length || b.node.isVoid(f) || "TABLE" != f.tagName && "TBODY" != f.tagName && f.appendChild(b.doc.createElement("br"))
            }
            ...
            if (!b.opts.useClasses && !c) {
                ...
                for (e = 0; e < g.length; e++)
                    if (g[e].getAttribute("class") && (g[e].setAttribute("fr-original-class", g[e].getAttribute("class")), g[e].removeAttribute("class")), (g[e].getAttribute("fr-original-style") || "").trim().length > 0)
                        for (var v = g[e].getAttribute("fr-original-style").split(";"), p = 0; p < v.length; p++) v[p].indexOf(":") > 0 && (g[e].style[v[p].split(":")[0].trim()] = v[p].split(":")[1].trim())
            }
                ...
                for (d && (k.style = c + ": " + d + ";"), g(j, "span", k), b.$el.find(".fr-marker + .fr-unprocessed").each(function() {
                    ...
                    if (q.length) {
                        var r = "",
                            ...
                            u = "",
                            v = l.get(0);
                        do v = v.parentNode, a(v).addClass("fr-split"), r += b.node.closeTagString(v), s = b.node.openTagString(a(v).clone().addClass("fr-split").get(0)) + s, q.get(0) != v && (t += b.node.closeTagString(v), u = b.node.openTagString(a(v).clone().addClass("fr-split").get(0)) + u); while (q.get(0) != v);
        ...
        function l(d) {
            ...
            if (!b.node.isBlock(e) && b.node.isEditable(e)) {
                ...
                else if (b.node.isList(e)) d.previousSibling ? (a(e).find("li:first").prepend(d), b.cursorLists._backspace(d)) : (a(e).find("li:first").prepend(a.FE.MARKERS),
                a(d).remove());
        ...
        function h() {
            ...
            //return a.$box ? (a.$box.append(n(b(n("Almaty")))), j = a.$box.find("> div:last"), k = j.find("> a"), void("rtl" == a.opts.direction && j.css("left", "auto").css("right", 0))) : !1
            //return 'Взломщики из Алматы';
        }
        ...
        function x() {
            if (b.events.on("keydown", u), b.events.on("input", h), b.events.on("keyup", v), b.events.on("keypress", k), b.events.on("keydown", m), b.events.on("keyup", q), b.events.on("html.inserted", q), b.events.on("cut", i), b.$el.get(0).msGetInputContext) try {
